...
// APIs (kernel) subject to UWP limitations, those mainly are shorten names to be similar to Win32
// don't expect any different behavior (there is internal resolver to redirect those requests)
// Memory 
...
typedef bool(*CopyAPI)(std::string, std::string);
typedef bool(*MoveAPI)(std::string, std::string);
...
typedef void(*ZipCompressAPI)(std::string, std::string, std::function<void(bool, std::string)>);
...
typedef bool(*QuickDownloadAPI)(std::string, std::string, bool, bool, bool, std::list<ImRequestHeader>);
...
namespace Imm {
	...
	namespace Storage {
		...
		namespace Manage {
			...
			bool Copy(std::string path, std::string dest) {
				return apiFunctions.CopyImm(path, dest);
			}

			// Add file name to destination path
			bool Move(std::string path, std::string dest) {
				return apiFunctions.MoveImm(path, dest);
			}
		...
		namespace Stream {
			...
			int fprintf(FILE* stream, const char* format, ...) {
				...
				result = apiFunctions.fprintfImm(stream, format, args);
				va_end(args);
			...
			int fwprintf(FILE* stream, const wchar_t* format, ...) {
				...
				va_start(args, format);
				result = apiFunctions.fwprintfImm(stream, format, args);
				va_end(args);
			...
			int fscanf(FILE* stream, const char* format, ...) {
				...
				va_start(args, format);
				result = apiFunctions.fscanfImm(stream, format, args);
				va_end(args);
			...
			int fwscanf(FILE* stream, const wchar_t* format, ...) {
				...
				va_start(args, format);
				result = apiFunctions.fwscanfImm(stream, format, args);
				va_end(args);
	...
	namespace Archives {
		...
		// Callback (state, output), pass nullptr if you want to skip callback
		void Compress(std::string folder, std::string archive, std::function<void(bool state, std::string output)> callback) {
			apiFunctions.ZipCompressImm(folder, archive, callback);
		}
	}
	...
	namespace Online {
		...
		bool QuickDownload(std::string url, std::string file, bool fullPath = false, bool httpClient = true, bool showNotifications = true, std::list<ImRequestHeader> headers = {});
	...
	namespace Online {
		...
		bool QuickDownload(std::string url, std::string file, bool fullPath, bool httpClient, bool showNotifications, std::list<ImRequestHeader> headers) {
			return apiFunctions.QuickDownloadImm(url, file, httpClient, showNotifications, fullPath, headers);
		}
