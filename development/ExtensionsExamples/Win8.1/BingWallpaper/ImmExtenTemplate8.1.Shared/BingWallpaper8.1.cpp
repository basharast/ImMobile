...
public:
	...
	// GamePad reporting [NOT SUPPORTED IN 8.1]
	...
	// GamePad (Analog) reporting [NOT SUPPORTED IN 8.1]
	...
	// GamePad connected [NOT SUPPORTED IN 8.1]
	...
	// GamePad removed [NOT SUPPORTED IN 8.1]
...
#pragma region APIsTests
	...
	void ImmTests(API_TESTS test) override {
		...
		{
			// Dialog confirm
			Imm::Dialogs::Confirm("Test Confirm", std::string("This is test confirm dialog"), [&](bool confirmed) {
				if (confirmed) {
		...
		case ONLINE_GITHUB_CONTENT:
		{
			...
			if (Imm::GitHub::GetContents("basharast", "wut", "", content)) {
				...
				// Parse example
				picojson::value contentJson;
				picojson::parse(contentJson, content);

				for (const auto& itemVal : contentJson.get<picojson::array>()) {
					const auto& item = itemVal.get<picojson::object>();

					std::string name = item.at("name").get<std::string>();
					std::string path = item.at("path").get<std::string>();
					std::string type = item.at("type").get<std::string>();
					std::string html_url = item.at("html_url").get<std::string>();
					std::string download_url = item.find("download_url") != item.end() && !item.at("download_url").is<picojson::null>()
						? item.at("download_url").get<std::string>()
						: "";
					int64_t size = item.count("size") ? static_cast<int64_t>(item.at("size").get<double>()) : 0;
				}
			}
			else {
		...
		case ONLINE_GITHUB_RELEASES:
		{
			...
			if (Imm::GitHub::GetLatestRelease("basharast", "wut", releases)) {
				...
				// Parse example
				picojson::value releasesJson;
				picojson::parse(releasesJson, releases);

				const auto& obj = releasesJson.get<picojson::object>();
				std::string releaseNote = obj.at("body").get<std::string>();
				std::string latestTag = obj.at("tag_name").get<std::string>();

				for (const auto& assetVal : obj.at("assets").get<picojson::array>()) {
					const auto& asset = assetVal.get<picojson::object>();

					std::string name = asset.at("name").get<std::string>();
					std::string download_url = asset.at("browser_download_url").get<std::string>();
					int64_t size = static_cast<int64_t>(asset.at("size").get<double>());
					int download_count = static_cast<int>(asset.at("download_count").get<double>());
					std::string content_type = asset.at("content_type").get<std::string>();
					std::string created_at = asset.at("created_at").get<std::string>().substr(0, 10);
					std::string updated_at = asset.at("updated_at").get<std::string>().substr(0, 10);
				}
			}
			else {
